...
import { ArrowDownLeft, Plus, Trash2, CheckCircle, AlertCircle, Save, Calculator, X, Calendar, Wallet } from 'lucide-react';
...
export const ReceiptVoucherForm: React.FC<ReceiptVoucherFormProps> = ({ currentUser, onSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!cashAccountCode) {
      setErrorMessage('يرجى اختيار صندوق أو بنك الاستلام');
      return;
    }

    if (!description.trim()) {
      setErrorMessage('يرجى إدخال البيان العام لسند القبض');
      return;
    }

    if (items.some((it) => !it.account_code || it.amount <= 0 || it.exchange_rate <= 0)) {
      setErrorMessage('يرجى التأكد من تعبئة جميع بنود المقبوضات بحساب صحيح ومبلغ وسعر صرف أكبر من صفر');
      return;
    }
    ...
    <div className="w-full max-w-5xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-emerald-600 dark:bg-emerald-500 text-white rounded-xl shadow-md shrink-0">
            <ArrowDownLeft className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-slate-950 dark:text-white">
              سند قبض نقدي جديد (Cash Receipt)
            </h2>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
              إثبات استلام النقدية في الصندوق مع قيد دائن مقابل لحساب العميل أو الإيراد
            </p>
          </div>
        </div>

        <div className="flex items-center gap-3 self-end sm:self-center font-mono">
          <div className="bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800/80 px-3.5 py-1.5 rounded-xl text-right">
            <span className="text-[10px] text-emerald-800 dark:text-emerald-300 font-bold block">رقم السند القادم</span>
            <span className="text-base sm:text-lg font-black text-emerald-700 dark:text-emerald-400">
              #{db.getNextVoucherNumber(1)}
            </span>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4">
          <h3 className="font-bold text-slate-900 dark:text-white text-sm border-b border-slate-100 dark:border-slate-800 pb-2.5 flex items-center gap-2">
            ...
            <span>البيانات الأساسية للصندوق والاستلام</span>
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100">
                صندوق الاستلام (حساب الصندوق المدين) <span className="text-rose-600">*</span>:
              </label>
              <select
                required
                value={cashAccountCode}
                onChange={(e) => setCashAccountCode(e.target.value)}
                className="w-full p-3 bg-slate-50 dark:bg-slate-950 border-2 border-slate-300 dark:border-slate-700 text-slate-950 dark:text-white font-bold text-xs sm:text-sm rounded-xl focus:border-emerald-500 dark:focus:border-emerald-400 focus:bg-white dark:focus:bg-slate-900 focus:outline-none transition-colors"
              >
                <option value="" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">-- اختر صندوق / بنك الاستلام --</option>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full p-3 bg-slate-50 dark:bg-slate-950 border-2 border-slate-300 dark:border-slate-700 text-slate-950 dark:text-white font-mono font-bold text-xs sm:text-sm rounded-xl focus:border-emerald-500 dark:focus:border-emerald-400 focus:bg-white dark:focus:bg-slate-900 focus:outline-none transition-colors"
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              required
              dir="auto"
              placeholder="مثال: استلام دفعة نقدية من التاجر عن رحلة رقم... / Cash payment received"
              value={description}
        ...
        {/* Dynamic Credit Items Container */}
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                يمكنك إضافة عدة بنود مقبوضات على نفس السند بعملات وأسعار صرف مختلفة
              </p>
            </div>
            ...
            <button
              ...
              className="self-start sm:self-auto px-4 py-2 bg-emerald-50 dark:bg-emerald-950/80 hover:bg-emerald-100 dark:hover:bg-emerald-900/80 text-emerald-800 dark:text-emerald-200 font-bold rounded-xl text-xs sm:text-sm flex items-center gap-1.5 border-2 border-emerald-300 dark:border-emerald-700 transition-colors shadow-sm cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              <span>إضافة بند دائن جديد</span>
            </button>
          </div>
            ...
            {items.map((item, idx) => (
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-slate-200/60 dark:border-slate-800/80 pb-2">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 rounded-lg text-xs font-black">
                    بند #{idx + 1}
                  </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-start">
                  ...
                  <div className="sm:col-span-4 space-y-1">
                    <label className="block text-[11px] font-bold text-slate-800 dark:text-slate-200">
                      الحساب الدائن (المقابل) <span className="text-rose-600">*</span>:
                    </label>
                    <select
                      required
                      value={item.account_code}
                      onChange={(e) => handleItemChange(idx, 'account_code', e.target.value)}
                      className="w-full p-2.5 bg-white dark:bg-slate-900 border-2 border-slate-300 dark:border-slate-700 text-slate-950 dark:text-white rounded-xl font-bold text-xs sm:text-sm focus:border-emerald-500 dark:focus:border-emerald-400 focus:outline-none"
                    >
                      <option value="" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">-- اختر الحساب المقابل --</option>
                  ...
                  <div className="sm:col-span-3 space-y-1">
                    ...
                    <input
                      ...
                      className="w-full p-2.5 bg-white dark:bg-slate-900 border-2 border-slate-300 dark:border-slate-700 text-slate-950 dark:text-white rounded-xl font-mono font-black text-sm sm:text-base focus:border-emerald-500 dark:focus:border-emerald-400 focus:outline-none"
                    />
                  </div>
                  ...
                  <div className="sm:col-span-3 space-y-1">
                    ...
                    <input
                      ...
                      className="w-full p-2.5 bg-white dark:bg-slate-900 border-2 border-slate-300 dark:border-slate-700 text-slate-950 dark:text-white rounded-xl font-mono font-bold text-xs sm:text-sm focus:border-emerald-500 dark:focus:border-emerald-400 focus:outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center pt-1">
                  <div className="sm:col-span-8 space-y-1">
                    ...
                    <input
                      ...
                      className="w-full p-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-950 dark:text-white rounded-lg text-xs font-semibold placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-emerald-500 dark:focus:border-emerald-400 focus:outline-none"
                    />
                  </div>

                  <div className="sm:col-span-4 flex items-center justify-between sm:justify-end gap-2 bg-emerald-50/60 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60 px-3 py-2 rounded-xl">
                    <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300">المكافئ:</span>
                    <span className="font-mono font-black text-emerald-800 dark:text-emerald-300 text-sm sm:text-base">
          ...
          <div className="p-4 sm:p-5 bg-emerald-50 dark:bg-emerald-950/50 rounded-2xl border-2 border-emerald-300 dark:border-emerald-800/80 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-sm">
            <div className="flex items-center gap-2.5 text-emerald-950 dark:text-emerald-200 font-bold text-xs sm:text-sm">
              <Calculator className="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0" />
              <span>الإجمالي المكافئ لسند القبض بالعملة الأساسية ({basicCurrency.currency_name}):</span>
            </div>
            <div className="text-xl sm:text-3xl font-black font-mono text-emerald-800 dark:text-emerald-300 tracking-tight">
              {totalEquivalent.toLocaleString()}{' '}
              <span className="text-xs sm:text-sm font-normal text-emerald-900 dark:text-emerald-200">
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 pt-2">
          ...
          <button
            type="submit"
            className="w-full sm:w-auto px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl text-sm transition-all shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2 active:scale-95 cursor-pointer"
          >
            <Save className="w-4 h-4" />
            <span>حفظ وترحيل سند القبض</span>
          </button>
        </div>
      </form>
    </div>
