...
import { ArrowUpRight, Plus, Trash2, CheckCircle, AlertCircle, Save, Calculator } from 'lucide-react';
...
export const PaymentVoucherForm: React.FC<PaymentVoucherFormProps> = ({ currentUser, onSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!description.trim()) {
      setErrorMessage('يرجى إدخال الوصف العام لسند الصرف');
      return;
    }

    if (items.some((it) => !it.account_code || it.amount <= 0 || it.exchange_rate <= 0)) {
      setErrorMessage('يرجى التأكد من تعبئة جميع بنود المصروفات بمبالغ وأسعار صرف أكبر من صفر');
      return;
    }
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">سند صرف نقدي جديد (Cash Payment)</h2>
            <p className="text-xs text-slate-500 mt-0.5">
              تسجيل المبالغ المدفوعة نقدياً من الصندوق مع إثبات القيود المدينة للموردين أو المصروفات فورياً.
            </p>
          </div>
        </div>
        ...
        <div className="text-left font-mono">
          ...
          <div className="text-lg font-bold text-rose-700">#{db.getNextVoucherNumber(2)}</div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <div className="p-4 bg-rose-50 rounded-xl border border-rose-200/80 flex items-center justify-between">
            <div className="flex items-center gap-2 text-rose-900 font-bold text-xs sm:text-sm">
              <Calculator className="w-5 h-5 text-rose-600" />
              <span>إجمالي صرف النقدية المكافئ بالعملة الأساسية:</span>
            </div>
            <div className="text-lg sm:text-2xl font-black font-mono text-rose-800">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button
            type="submit"
            className="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl text-sm transition-colors shadow-md flex items-center gap-2 active:scale-95"
          >
            <Save className="w-4 h-4" />
            <span>حفظ سند الصرف المباشر</span>
          </button>
        </div>
      </form>
    </div>
